...
for (const codec of codecs) {
  test.describe(`Host-Product E2E [${codec}]`, () => {
    ...
    test('error class: signPayload rejected produces SigningErr.Rejected', async ({ page }) => {
      const frame = await getProductFrame(page);
      const info = await runProductTest(frame, 'errorClass_signPayloadRejected') as Record<string, unknown>;
      expect(info.isError).toBe(true);
      expect(info.name).toBe('SigningErr::Rejected');
      expect(info.message).toBe('Rejected');
      expect(info.instance).toBe('Rejected');
      expect(info.tag).toBe('Rejected');
      expect(info.payload).toBeUndefined();
      expect(info.value).toBeUndefined();
      expect((info.instanceOfChecks as Record<string, boolean>)['SigningErr.Rejected']).toBe(true);
      expect((info.instanceOfChecks as Record<string, boolean>)['SigningErr.Unknown']).toBe(false);
      expect((info.instanceOfChecks as Record<string, boolean>)['Error']).toBe(true);
    });

    test('error class: createTransaction NotSupported produces CreateTransactionErr.NotSupported', async ({ page }) => {
      const frame = await getProductFrame(page);
      const info = await runProductTest(frame, 'errorClass_createTransaction') as Record<string, unknown>;
      expect(info.isError).toBe(true);
      expect(info.name).toBe('CreateTransactionErr::NotSupported');
      expect(info.message).toBe('Not Supported');
      expect(info.instance).toBe('NotSupported');
      expect(info.tag).toBe('NotSupported');
      expect(info.payload).toBe('Not implemented in E2E');
      expect(info.value).toBe('Not implemented in E2E');
      expect((info.instanceOfChecks as Record<string, boolean>)['CreateTransactionErr.NotSupported']).toBe(true);
      expect((info.instanceOfChecks as Record<string, boolean>)['CreateTransactionErr.Rejected']).toBe(false);
      expect((info.instanceOfChecks as Record<string, boolean>)['Error']).toBe(true);
    });

    test('error class: navigateTo blocked produces NavigateToErr.PermissionDenied', async ({ page }) => {
      const frame = await getProductFrame(page);
      const info = await runProductTest(frame, 'errorClass_navigateToBlocked') as Record<string, unknown>;
      expect(info.isError).toBe(true);
      expect(info.name).toBe('NavigateToErr::PermissionDenied');
      expect(info.message).toBe('Permission denied');
      expect(info.instance).toBe('PermissionDenied');
      expect(info.tag).toBe('PermissionDenied');
      expect((info.instanceOfChecks as Record<string, boolean>)['NavigateToErr.PermissionDenied']).toBe(true);
      expect((info.instanceOfChecks as Record<string, boolean>)['NavigateToErr.Unknown']).toBe(false);
    });

    test('error class: storageWrite full produces StorageErr.Full', async ({ page }) => {
      const frame = await getProductFrame(page);
      const info = await runProductTest(frame, 'errorClass_storageWriteFull') as Record<string, unknown>;
      expect(info.isError).toBe(true);
      expect(info.name).toBe('StorageErr::Full');
      expect(info.message).toBe('Storage is full');
      expect(info.instance).toBe('Full');
      expect((info.instanceOfChecks as Record<string, boolean>)['StorageErr.Full']).toBe(true);
      expect((info.instanceOfChecks as Record<string, boolean>)['StorageErr.Unknown']).toBe(false);
    });

    test('error class: accountGetAlias Unknown with reason', async ({ page }) => {
      const frame = await getProductFrame(page);
      const info = await runProductTest(frame, 'errorClass_accountGetAlias') as Record<string, unknown>;
      expect(info.isError).toBe(true);
      expect(info.name).toBe('RequestCredentialsErr::Unknown');
      expect(info.message).toBe('RequestCredentials: unknown error');
      expect(info.instance).toBe('Unknown');
      expect(info.tag).toBe('Unknown');
      expect(info.payload).toMatchObject({ reason: 'Not supported' });
      expect((info.instanceOfChecks as Record<string, boolean>)['RequestCredentialsErr.Unknown']).toBe(true);
      expect((info.instanceOfChecks as Record<string, boolean>)['RequestCredentialsErr.Rejected']).toBe(false);
    });

    test('error class: accountCreateProof Unknown with reason', async ({ page }) => {
      const frame = await getProductFrame(page);
      const info = await runProductTest(frame, 'errorClass_accountCreateProof') as Record<string, unknown>;
      expect(info.isError).toBe(true);
      expect(info.name).toBe('CreateProofErr::Unknown');
      expect(info.message).toBe('CreateProof: unknown error');
      expect(info.instance).toBe('Unknown');
      expect(info.payload).toMatchObject({ reason: 'Not supported' });
      expect((info.instanceOfChecks as Record<string, boolean>)['CreateProofErr.Unknown']).toBe(true);
      expect((info.instanceOfChecks as Record<string, boolean>)['CreateProofErr.Rejected']).toBe(false);
    });

    test('error class: chatCreateRoom PermissionDenied', async ({ page }) => {
      const frame = await getProductFrame(page);
      const info = await runProductTest(frame, 'errorClass_chatCreateRoom') as Record<string, unknown>;
      expect(info.isError).toBe(true);
      expect(info.name).toBe('ChatRoomRegistrationErr::PermissionDenied');
      expect(info.message).toBe('Permission denied');
      expect(info.instance).toBe('PermissionDenied');
      expect((info.instanceOfChecks as Record<string, boolean>)['ChatRoomRegistrationErr.PermissionDenied']).toBe(true);
    });

    test('error class: chatPostMessage Unknown with reason', async ({ page }) => {
      const frame = await getProductFrame(page);
      const info = await runProductTest(frame, 'errorClass_chatPostMessage') as Record<string, unknown>;
      expect(info.isError).toBe(true);
      expect(info.name).toBe('ChatMessagePostingErr::Unknown');
      expect(info.message).toBe('ChatMessagePosting: unknown error');
      expect(info.instance).toBe('Unknown');
      expect(info.payload).toMatchObject({ reason: 'disabled' });
      expect((info.instanceOfChecks as Record<string, boolean>)['ChatMessagePostingErr.Unknown']).toBe(true);
    });

    test('error class: statementStoreCreateProof Unknown with reason', async ({ page }) => {
      const frame = await getProductFrame(page);
      const info = await runProductTest(frame, 'errorClass_statementStoreCreateProof') as Record<string, unknown>;
      expect(info.isError).toBe(true);
      expect(info.name).toBe('StatementProofErr::Unknown');
      expect(info.message).toBe('StatementProof: unknown error');
      expect(info.instance).toBe('Unknown');
      expect(info.payload).toMatchObject({ reason: 'disabled' });
      expect((info.instanceOfChecks as Record<string, boolean>)['StatementProofErr.Unknown']).toBe(true);
    });
